...
def save_image(image, model, description):
    ...
    conf *= 100
    img_name = f'fgsm_output/{description}, {label} ({conf:.2f}%).jpg'
...
# pretrained_model = InceptionV3(include_top=True, weights='imagenet')
pretrained_model.trainable = False

image = tf.io.read_file('tabby_cat.jpg')
...
for epsilon in [0, 0.005, 0.01, 0.1, 0.15, 0.2]:
    ...
    save_image(corrupted_image, pretrained_model, f'Epsilon = {epsilon:.3f}')